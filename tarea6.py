...
#lee el file y almacena el contenido en un diccionario, para luego usar el metodo de agregar paciente de la clase clinica para cargarlos

try:
    if os.stat('clinica.txt').st_size > 0:
        with open('clinica.txt') as f:
            persona = json.load(f)
        f.close()

        for individuo in persona.keys():
            la_clinica.agregar_paciente(individuo, persona[individuo]['Nombre'], persona[individuo]['Apellido'],
                                        persona[individuo]['teléfono'], persona[individuo]['dirección'],
                                        persona[individuo]['lista_de_enfermedades_tratadas'],
                                        persona[individuo]['lista_de_medicamentos_que_toma'])
    else:
        print('No hay lista de pacientes del día anterior')
except OSError:
    print('****No existe el file de clinica****')
...
'''''''''
luci = {'identificación':4,'Nombre':'Luciana','Apellido':'Salas','teléfono':44581154,
'dirección':'Heredia 5 st, 9 Av','lista_de_enfermedades_tratadas' : ['Locura', 'Calentura'],
'lista_de_medicamentos_que_toma': ['amanza locos', 'ibuprofeno']}
'''
#Agregando pacientes
...
#la_clinica.agregar_paciente(**luci)
...
#copia todo el contenido del diccionario al archivo de texto serializado

with open('clinica.txt', 'w+') as f:
    json.dump(la_clinica, f)
    f.close()
